...
/**
 * @description: 如果 first 指定属性为 null 或 undefined， 则从 second 中提取
 ...
 * @return {*}
 */
export function chainSelect(object: object, chain?: string) {
  let target = object

  if (chain) {
    chain.split('.').forEach((link) => {
      target = target[link] || ''
    })
  }
...
// 把对象拼接成字符串，以便小程序不支持styleObject的问题
export function styleToString(...args: StyleProp[]): string {
  const styles = []
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (typeof arg === 'string') {
      ...
      Object.keys(arg).forEach((key) => {
        if (arg[key]) {
          styles.push(`${kebabCase(key)}:${arg[key]}`)
        }
      })
    }
  }
...
export type ClassProp = string | number | Record<string, any> | ClassProp[]
