...
| edit-method <sup>1.25.10+</sup>    | 自定义编辑方法，用于处理节点的增删改操作                                     | (params: {editType: string, editNode: TreeStateNode, editValue: string, parentNode?: TreeStateNode}) => Promise<{errMsg?: string, key?: string}> | -               |
| edit-options <sup>1.25.10+</sup>   | 自定义编辑菜单选项，用于替换默认的编辑菜单项                                 | MenuOption[]                                                                                                                                     | -               |
